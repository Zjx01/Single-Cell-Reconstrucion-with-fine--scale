...
def val_model(opt,val_loader,model,criterion,vis_tool):
    ...
    for batch_ids,(data,target) in enumerate(val_loader):
        if opt.use_cuda:
            ...
            with torch.no_grad():
                ...
                # begin to show the result
                if batch_ids % opt.val_plotfreq == 0:
                    vis_tool.plot('Val_Loss', loss.item())
                    vis_tool.plot('Val_Dice', dice_value)
                    vis_tool.plot('Val_Recall', recall_value)
